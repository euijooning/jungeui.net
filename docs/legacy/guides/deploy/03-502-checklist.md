...
- **new-admin.jungeui.net** (또는 백오피스 도메인)용 `server` 블록에서 `location /api/` 찾기.
  - **`proxy_pass http://127.0.0.1:XXXX;`** 의 포트 `XXXX`가 **systemd에서 API가 리스닝하는 포트와 동일**한지 확인 (스테이징 예: 8020).
...
서버에서 (API 포트가 8020이라 가정):

```bash
curl -v -X POST http://127.0.0.1:8020/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"admin@jungeui.net","password":"비밀번호"}'
```

- **127.0.0.1:8020** 에서 정상 응답이 나오면 → API는 동작하는 것이고, **nginx 쪽 proxy_pass/포트 문제**일 가능성이 큼.
