...
| `API_PORT` | API 리스닝 주소 (http:// 포함) | `http://127.0.0.1:8020` |
...
server {
    ...
    location /api/ {
        proxy_pass API_PORT;
        client_max_body_size 25M;   # 파일 업로드 (assets/upload 등) 413 방지
...
server {
    ...
    location /api/ {
        proxy_pass API_PORT;
        client_max_body_size 25M;   # 파일 업로드 413 방지
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
...
**주의**:
- `proxy_pass`에는 `http://127.0.0.1:8020` 형태로 프로토콜·포트까지 포함. 경로만 쓰면 안 됨.
- `location /api/` → `proxy_pass http://127.0.0.1:8020` 인 경우, `/api/posts` 요청이 `http://127.0.0.1:8020/api/posts`로 전달됨 (경로 유지).
- 설정 반영: `sudo nginx -t`로 문법 검사 후 `sudo systemctl reload nginx`.
- **413 (Content Too Large)** 가 나오면: `location /api/` 블록에 `client_max_body_size 25M;` 이 있는지 확인. 없으면 추가 후 `sudo systemctl reload nginx`.
