...
- API 연동: 루트 `.env`의 `VITE_API_URL`(예: `http://localhost:1217`) 사용. 백오피스 Vite 프록시가 `/api`를 해당 URL로 넘깁니다.
...
| 터미널 | 위치 | 명령 |
|--------|------|------|
| 1 | 프로젝트 루트 | `uvicorn apps.api.main:app --reload --port 1217` |
| 2 | `apps/backoffice` | `npm run dev` |
| 3 | `apps/client` | `npm run dev` |

- API: http://localhost:1217  
