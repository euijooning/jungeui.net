...
- **UI 변경**: 두 번째 카드 "발행 포스트"(PUBLISHED + UNLISTED). 아이콘 `fa-file-alt`, 값은 `stats.published_posts`.
- **API 추가**: `GET /api/dashboard/stats`, `GET /api/dashboard/recent-activity`
  ...
  - **방문자/조회수**: `daily_stats` 테이블에서 `date` 기준 오늘 `visitor_count`, 전일까지 `SUM(total_views)`.
...
- **알림**: `/notifications` 페이지, AppAppBar 프로필 왼쪽 종 아이콘.
...
| 알림 | `pages/notifications/NotificationsPage.jsx`, `App.jsx` (/notifications), `AppAppBar.jsx` (종 아이콘) |

---

## 7. Phase 02 구현 요약

- **포스트 목록**: PostList. GET /api/posts(필터 category/tag/status, 제목 검색 q, 정렬 order_by, 페이지네이션). 테이블(번호·제목·카테고리·상태·등록일·발행일·작업). [새 포스트], 일괄 [삭제], 행별 [보기/수정/삭제].
- **포스트 에디터·상세**: PostEditor(PostNew/PostEdit), PostDetail. Toast UI Editor, 설정 패널(상태·발행 방식·발행일·카테고리·태그·썸네일). 이미지 업로드·유튜브 URL 임베드. 첨부파일 다중 업로드(png, jpg, pdf, ppt, hwp, docx 등 10MB)·목록·삭제·저장 시 attachment_asset_ids 전송. 상세 화면에서 첨부 목록 표시·다운로드(GET /api/assets/:id/download).
- **API**: POST/PUT/GET/DELETE /api/posts, attachment_asset_ids·attachments, GET /api/assets/:id/download. 상세는 [docs/backoffice/README.md](../../docs/backoffice/README.md) 참고.
