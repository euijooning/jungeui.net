...
- **PUT 수정**: `parent_id`를 **null로 설정**(대카테고리로 승격) 가능해야 함. 요청 body에 `parent_id` 키가 있을 때만 갱신하도록 처리 (예: Pydantic `model_fields_set`에 `"parent_id"` 포함 여부로 판단). `if body.parent_id is not None`만 쓰면 null 전송 시 갱신이 스킵되므로 주의.
...
- **기능**:
  - 추가: 대카테고리 추가 / 선택한 대 아래 소카테고리 추가. **소카테고리 행에는 [추가](+) 버튼 없음** (하위 추가는 대카테고리만).
  - 수정: 이름·slug·parent_id·sort_order.
  - 삭제: 하위 있으면 경고 후 cascade 또는 거부 정책에 따라 처리.
  - **드래그 앤 드롭**:
    - 같은 레벨 내 순서 변경 + **소카테고리 → 대카테고리로 드래그 이동**(대카테고리화) 지원. 드롭 위치는 **초록색 세로선**으로 표시.
    - 저장 시: 부모가 바뀐 항목은 먼저 `PUT /api/categories/{id}`로 `parent_id` 반영 후, `PATCH /api/categories/reorder` 호출.
  - **순서 저장**: 변경이 없을 때는 버튼 비활성화, 변경 시에만 활성화(초록색). 저장 완료 시 "순서가 저장되었습니다" 모달 표시.
  - **이탈 경고**: react-admin은 자체 라우터를 사용해 `useBlocker`와 궁합이 맞지 않아, 인앱(사이드바) 이동은 막지 않음. **탭/창 닫을 때만** `beforeunload`로 브라우저 기본 경고.
...
- **표시 규칙**: 목록에서는 소카테고리를 " — 소이름"(em dash + 공백)으로 구분. **선택된 값**은 "대이름 > 소이름" 형태로 표시.
...
- 소카테고리를 대카테고리로 드래그 이동(대카테고리화) 시 저장 반영. (API PUT에서 `parent_id: null` 허용)
- 글 쓰기/글 목록에서 대→소 구조로 카테고리 선택 가능(목록 " — 소이름", 선택값 "대 > 소").
