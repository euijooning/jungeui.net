...
CREATE TABLE IF NOT EXISTS project_tag_labels (
  project_id BIGINT NOT NULL,
  sort_order INT NOT NULL DEFAULT 0 COMMENT '노출 순서',
  name VARCHAR(50) NOT NULL COMMENT '프로젝트 소개용 태그명',
  PRIMARY KEY (project_id, sort_order),
  FOREIGN KEY (project_id) REFERENCES projects(id) ON DELETE CASCADE
) COMMENT='프로젝트 소개용 태그 (프로젝트별 독립)';
            ...
            if cur.fetchone() is None:
                cur.execute("""
                    CREATE TABLE site_settings (
                      ...
                      value TEXT NULL COMMENT '값 (URL, 소개 문구 등)',
                      updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
                    ) COMMENT='사이트 설정 (키-값)'
                """)
                conn.commit()
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                ...
                "WHERE TABLE_SCHEMA = %s AND TABLE_NAME = 'site_settings' AND COLUMN_NAME = 'value'",
                (MYSQL_DATABASE,),
            )
            row = cur.fetchone()
            if row and row[0] and row[0].upper() == "VARCHAR":
                cur.execute(
                    "ALTER TABLE site_settings MODIFY COLUMN value TEXT NULL "
                    "COMMENT '값 (URL, 소개 문구 등)'"
                )
                conn.commit()
                logger.info("site_settings.value 컬럼을 TEXT로 변경함")
    finally:
        conn.close()
