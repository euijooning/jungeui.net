      ...
      <div className="px-10 py-8 flex-1 flex flex-col min-h-0 min-w-0">
        ...
        <div className="flex flex-wrap gap-1.5 mb-6 overflow-hidden">
          {tags.map((t) => (
            <span
              key={t.id}
              className="text-[0.75rem] px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-600 max-w-full truncate"
            >
              {t.name}
            </span>
          ))}
        ...
        <p className="text-[1rem] theme-text-secondary line-clamp-3 leading-relaxed wrap-break-word overflow-hidden min-w-0">
          {desc || ''}
        </p>

        {/* 7. 상세이미지: 등록 시에만 영역 표시 */}
        {project.intro_image && (
          <div className="w-full aspect-2/1 rounded-lg overflow-hidden mt-3 bg-gray-100 dark:bg-gray-800 shrink-0 flex items-center justify-center">
            <img
