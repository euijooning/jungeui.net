...
export default function About() {
  ...
  useEffect(() => {
    let cancelled = false;
    fetchTags({ used_in_posts: true }).then((list) => {
  ...
  return (
    <SharedLayout categories={[]}>
      ...
      <section className="relative left-1/2 -translate-x-1/2 w-screen py-10 md:py-14 bg-[#F0F9FF] overflow-hidden -mb-8">
        <div className="w-full max-w-[1200px] mx-auto px-4 md:px-6">
        ...
        {projects.length > 0 ? (
          <div className="flex flex-col items-center w-full relative">
            ...
            <div
              className={`hidden md:block absolute left-0 top-0 bottom-0 w-16 md:w-32 z-10 bg-linear-to-r from-[#F0F9FF] to-transparent pointer-events-none transition-opacity duration-300 ${
                carouselIndex === 0 ? 'opacity-0' : 'opacity-100'
              }`}
            />
            <div
              className={`hidden md:block absolute right-0 top-0 bottom-0 w-16 md:w-32 z-10 bg-linear-to-l from-[#F0F9FF] to-transparent pointer-events-none transition-opacity duration-300 ${
